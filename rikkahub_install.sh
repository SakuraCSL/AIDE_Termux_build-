...
# ============================================================
# Android 开发环境网络安装脚本（RikkaHub / Debian/Ubuntu 容器）
...
fix_aapt2() {
    ...
    if ! command -v aapt2 >/dev/null 2>&1; then
        log "  安装 aapt2..."
        apt-get install -y aapt2 || { log "  警告：aapt2 安装失败，资源编译可能异常"; return 1; }
    else
        log "  aapt2 已安装 ✓"
    fi
...
network_install() {
    ...
    # 写入新配置
    cat >> "$RC_FILE" << 'EOF'
# Android Development Environment (RikkaHub Install)
